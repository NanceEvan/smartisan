    ...
    <div class="account-order">
      <div class="gray-box">
        ...
        <div class="js-list-inner">
          <div class="box-inner order-cart order-list-cart clear" v-for="(orderItem, index) in orderData" v-bind:key="index">
            <div class="gray-sub-title cart-title">
              <span class="date">2017-06-19</span>
              <span class="order-id"> 订单号： <router-link :to="{'name': 'Payment', query:{'orderId':orderItem.orderId}}">{{orderItem.orderId}}</router-link> </span>
              <span class="order-detail"><router-link :to="{'name': 'Payment', query:{'orderId':orderItem.orderId}}">查看详情&gt;</router-link> </span> <span class="sub-total">应付总额</span>
              <span class="num">数量</span>
              <span class="price">单价</span>
            </div>
            <div class="cart">
              <div class="cart-items clear" v-for="(goodItem, index) in orderItem.goodsData" v-bind:key="index">
                <div class="prod-info clear">
                  <div class="items-thumb">
                    <a href="javascript:;" target="_blank"><img :src="goodItem.ali_image + '?x-oss-process-image/resize,w_80/quality,Q_100/format,webp'"></a>
                  </div>
                  <div class="items-params clear">
                    <div class="name vh-center">
                      <a :href="'localhost/item?itemId=' + goodItem.sku_id" :title="goodItem.title+'('+goodItem.item+')'+goodItem.spec_json.show_name" target="_blank">{{goodItem.title+'('+goodItem.item+')'+goodItem.spec_json.show_name}}</a>
                    </div>
                    <div class="detail"></div>
                  </div>
                  <div class="num">{{goodItem.count}}</div>
                  <div class="price">¥ {{getPriceFixed2(goodItem.price)}}</div>
                </div>
              </div>
            </div>
            <div class="prod-operation">
              <div class="total">¥ {{getPriceFixed2(orderItem.price)}}</div>
              <div class="status">
                <router-link class="blue-small-btn js-payment-order" :to="{'name': 'Payment', query:{'orderId':orderItem.orderId}}" v-if="!orderItem.isPay">现在付款</router-link>
                <router-link :to="{'name': 'Payment', query:{'orderId':orderItem.orderId}}" v-if="orderItem.isPay">已完成</router-link>
              </div>
            </div>
          </div>
        </div>
