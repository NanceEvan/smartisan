<template>
  <div class="content page-order-payment">
    <div class="gray-box clear">
      ...
      <div class="box-inner payment-checkout-panel clear">
        <span class="jianguo-blue-main-btn big-main-btn js-payment-order" @click="payNowHandle">
            <a v-if="!orderInfo.isPay">现在支付</a>
            <a v-else>已支付</a>
        </span>
        <span class="prices"> 应付金额：   <em><span>¥ </span>{{orderInfo.price}}</em>   </span>
      </div>
    </div>
    ...
    <div class="confirm-detail">
      ...
      <p class="info-detail">联系电话：{{orderInfo.receiveInfo.phone}}</p>  <p class="info-detail">详细地址：{{orderInfo.receiveInfo.province+orderInfo.receiveInfo.city+orderInfo.receiveInfo.county+orderInfo.receiveInfo.add}}</p>
    </div>
    <div class="confirm-detail">
    ...
    <div class="confirm-goods-table">
      <div class="cart-items clear" v-for="(goodsItem, index) in orderInfo.goodsData" v-bind:key="index">
        <div class="name fn-left hide-row">
          <div class="name-cell">
            <a :href="'localhost/item?itemId=' + goodsItem.sku_id" :title="goodsItem.title+'('+goodsItem.item+')'+goodsItem.spec_json.show_name" target="_blank">{{goodsItem.title+'('+goodsItem.item+')'+goodsItem.spec_json.show_name}}</a><br>
          </div>
        </div>
        <div class="subtotal fn-right">
          <div class="subtotal-cell">¥ {{subTotalPrice(goodsItem.price, goodsItem.count)}}</div>
        </div>
        <div class="goods-num fn-right">{{goodsItem.count}}</div>
        <div class="price fn-right">¥ {{getPriceFixed2(goodsItem.price)}}</div>
      </div>
    </div>
    <div class="order-discount-line">
      <p> 商品总计： <span>¥ {{orderInfo.price}}</span> </p>
      <p> 运费： <span>+ ¥ {{getPriceFixed2(orderInfo.freight)}}</span> </p>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    orderInfo () {
      return this.$store.state.orderData.filter((order) => {
  ...
  methods: {
    ...
    payNowHandle () {
      if (!this.orderInfo.isPay) {
        this.$store.commit('payNow', this.orderInfo.orderId)
