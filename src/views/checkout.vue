        ...
        <div class="box-inner js-checkout-address-panel ">
          <div class="address-common-table js-multiple-address-panel">
            <ul class="address-item-list clear js-address-item-list">
              <li class="js-choose-address" :class="{'selected-address-item': receiveIndex===index}" v-for="(addItem,index) in receiveInfo" v-bind:key="index" @click="changeReceive(index)">
                <div class="address-item">
                  <div class="name-section">  {{addItem.name}}  </div>
                  <div class="mobile-section">{{addItem.phone}}</div>
                  <div class="detail-section"> {{addItem.province}} {{addItem.city}} {{addItem.country}}<br> {{addItem.add}} </div>
                </div>
        ...
        <div class="invoice-detail"> 发票抬头：
          <div class="radio-box">
            <label>
              ...
              <span class="blue-radio" :class="{'blue-radio-on': invoice.personal}" @click="checkInvoice(true)"><a></a></span>  个人
            </label>
            <label>
              <input type="radio" class="hide">
              <span class="blue-radio" :class="{'blue-radio-on': !invoice.personal}" @click="checkInvoice(false)"><a></a></span>  单位
            </label>
          </div>
    ...
    <div class="gray-box">
      ...
      <div class="box-inner ui-goods-cart">
        ...
        <div class="cart-table">
          <div class="cart-group js-cart-group">
            <div class="cart-items" v-for="(item,index) in checkedGoods" v-bind:key="index">
              <div class="items-thumb">
                <a href="javascript:void(0)" target="_blank"><img :src="item.ali_image + '?x-oss-process=image/resize,w_80/quality,Q_100/format,webp'"></a>
              </div>
              <div class="name hide-row">
                <div class="name-cell">
                  <router-link :to="{name:'Item', query: {itemId:item.sku_id}}" target="_blank" :title="item.title"><a>{{item.title}}  ({{item.spec_json.show_name}})</a></router-link>
                </div>
              </div>
              <div class="subtotal">
                <div class="subtotal-cell">  ¥ {{subTotalPrice(item.price, item.count)}} </div>
              </div>
              <div class="goods-num">{{item.count}}</div>
              <div class="price">¥ {{getPriceFixed2(item.price)}}</div>
            </div>
          </div>
        </div>
      ...
      <div class="box-inner">
        <div class="order-discount-line">
          <p> 商品总计： <span>¥{{goodsCheckedTotalPrice}}</span> </p>
          <p> 运费： <span>+ ¥ {{freight}}</span> </p>
          <p class="discount-line js-discount-cash"> <em>现金券</em>： <span> - {{coupon}} </span> </p>
        </div>
      </div>
      <div class="box-inner">
        <div class="last-payment clear">
          <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click="submitOrderHandle"> <a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{transactionAmount(goodsCheckedTotalPrice, freight, coupon)}}</em></span>
        </div>
      </div>
    </div>
    <address-pop v-if="popSignal" @close="hideAddressPop"></address-pop>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    goodsCheckedTotalCount () {
      return this.$store.getters.goodsCheckedTotalCount
    },
    goodsCheckedTotalPrice () {
      return this.$store.getters.goodsCheckedTotalPrice
    },
    freight () {
      let f = 8
      if (this.goodsCheckedTotalPrice > 80) {
        f = 0
      }
      return f
    },
  ...
  methods: {
    subTotalPrice (price, count) {
      return NP.times(price, count).toFixed(2)
    },
    getPriceFixed2 (price) {
      return price.toFixed(2)
    },
    transactionAmount (totalPrice, freight, coupon) {
      return NP.plus(totalPrice, freight, -coupon).toFixed(2)
    },
    changeReceive (index) {
      this.receiveIndex = index
    },
    showAddressPop () {
      // this.$store.state.addressPopSignal = true
      this.popSignal = true
    },
    hideAddressPop () {
    ...
    submitOrderHandle () {
      ...
      // 订单号应该由前端发起一次请求， 后端生成订单号传给前端
      // 这里直接使用时间戳作为订单号
      let orderData = {
        orderId: iDate.getTime(),
        goodsData: this.checkedGoods,
        price: this.goodsCheckedTotalPrice,
      ...
      this.$store.commit('submitOrder', orderData)
      this.$router.push({path: 'payment', query: {orderId: orderData.orderId}})
    }
  }
}
</script>
