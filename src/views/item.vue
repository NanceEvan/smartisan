  ...
  <div class="item-box">
    <div class="gallery-wrapper">
      <div class="gallery">
        <div class="thumbnail">
          <ul>
            <li v-for="(t_img,index) in itemInfo.ali_images" v-bind:key="index" :class="{'on':index===imageIndex}" @mouseenter="tableImg(index)"><img :src="t_img + '?x-oss-process=image/resize,w_160/format,webp'"></li>
          </ul>
        </div>
        <div class="thumb">
          <ul>
            <li v-for="(t_img,index) in itemInfo.ali_images" v-bind:key="index" :class="{'on':index===imageIndex}"><img :src="t_img + '?x-oss-process=image/resize,w_1054/format,webp'"></li>
          </ul>
        </div>
      </div>
    </div>
    <div class="banner">
      ...
      <div class="sku-dynamic-params-panel">
        <div class="sku-dynamic-params clear">
          ...
          <ul class="params-colors">
            <li v-for="(sku,index) in itemInfo.sku_list" v-bind:key="index" :class="{'cur':sku.id==$route.query.itemId}">
              <router-link :title="sku.color" :to="{name:'Item', query:{itemId:sku.id}}">
                <i><img :src="'http://img01.smartisanos.cn/' + sku.image + '20X20.jpg'"></i>
              </router-link>
            </li>
          </ul>
        </div>
        <div class="sku-dynamic-params clear">
          ...
          <div class="params-detail clear">
            <div class="item-num js-select-quantity">
              <span class="down" :class="{'down-disabled':itemCount<=1}" @click="decreaseItemCount">-</span>
              <span class="num">{{itemCount}}</span>
              <span class="up" :class="{'up-disabled':itemCount>=itemInfo.limit_num}" @click="increaseItemCount">+</span>
            </div>
          </div>
        </div>
      </div>
      <div class="sku-status">
        <div class="cart-operation-wrapper clearfix">
          <span class="blue-title-btn js-add-cart" @click="addCarPanelHandle(itemCount)"><a>加入购物车</a></span>
  ...
  computed: {
    itemInfo () {
      return this.itemData.filter((item) => {
        return String(item.sku_id) === String(this.itemId)
      })[0]
    }
  },
  methods: {
    ...
    addCarPanelHandle (count) {
      let data = {'info': this.itemInfo, 'count': count}
      this.$store.commit('addCarPanelData', data)
    },
    increaseItemCount () {
      if (this.itemCount < this.itemInfo.limit_num) {
        this.itemCount++
      }
    },
    decreaseItemCount () {
