    ...
    <div class="store-content">
      <div class="cart-box">
        ...
        <div class="cart-inner">
          <div class="empty-label" v-if="goodsTotalCount < 1">
            <h3>您的购物车中还没有商品</h3>
          ...
          <div v-else>
            ...
            <div class="cart-table">
              <div class="cart-group">
                <!--购物列表-->
                <div class="cart-top-items" v-for="(item,index) in carPanelData" v-bind:key="index">
                  <div class="cart-items">
                    <div class="items-choose">
                      <span class="blue-checkbox-new" :class="{'checkbox-on': item.checked}" @click="turnCheckGoodsHandle(item.sku_id)"><a></a></span>
                    </div>
                    ...
                    <div class="operation">
                      <a class="items-delete-btn" @click="goodsRemoveHandle(item.sku_id)"></a>
                    </div>
                    <div class="subtotal">¥ {{subTotalPrice(item.price, item.count)}}</div>
                    <div class="item-cols-num">
                      <div class="select js-select-quantity">
                        <div class="select js-select-quantity">
                          <span class="down" :class="{'down-disabled': item.count<=1}" @click="decreaseCarpanelDataHandle(item.sku_id)">-</span>
                          <span class="num">{{item.count}}</span>
                          <span class="up" :class="{'up-disabled': item.count>=item.limit_num}" @click="increaseCarpanelDataHandle(item.sku_id)">+</span>
                        </div>
              ...
              <div class="cart-bar-operation">
                <div>
                  <div class="choose-all js-choose-all">
                    <span class="blue-checkbox-new" :class="{'checkbox-on': allChecked}" @click="checkAllHandle"><a></a></span>
                    全选
                  </div>
              ...
              <div class="shipping">
                <div class="shipping-box">
                  <div class="shipping-total shipping-num">
                    <h4 class="">
                      已选择 <i>{{goodsCheckedTotalCount}}</i> 件商品
                    </h4>
                    <h5>
                      共计 <i >{{goodsTotalCount}}</i> 件商品
                    </h5>
                  </div>
                  <div class="shipping-total shipping-price">
                    <h4 class="">
                      应付总额：<span>￥</span><i >{{goodsCheckedTotalPrice}}</i>
                    </h4>
                ...
                <span class="jianguo-blue-main-btn big-main-btn js-checkout" :class="{'disabled-btn': goodsCheckedTotalCount<=0}" @click="checkout"><a>现在结算</a></span>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    goodsTotalCount () {
      return this.$store.getters.goodsTotalCount
    },
    goodsTotalPrice () {
      return this.$sotre.getters.goodsTotalPrice
    },
  ...
  methods: {
    goodsRemoveHandle (id) {
      this.$store.commit('delCarPanelDate', id)
    },
    increaseCarpanelDataHandle (id) {
      this.$store.commit('increaseCarPanelData', id)
    },
    decreaseCarpanelDataHandle (id) {
      this.$store.commit('decreaseCarPanelData', id)
    },
    ...
    turnCheckGoodsHandle (id) {
      this.$store.commit('turnCheckGoods', id)
    },
    checkAllHandle () {
      this.$store.commit('allCheck', this.allChecked)
    },
    ...
    checkout () {
      // 编程式导航
      // 选中物品少于1时不可导航
      if (this.goodsCheckedTotalCount > 0) {
