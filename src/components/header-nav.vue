<template>
  <div id="header">
    <div class="nav-global">
      <div class="container">
        <h1 class="nav-logo">
          <a href="javascript:void(0)"></a>
        </h1>
        <ul class="nav-aside">
          <li class="nav-user">
            <a href="javascript:void(0)">个人中心</a>
            <!--active-->
            <div class="nav-user-wrapper">
              <div class="nav-user-list">
                <dl class="nav-user-avatar">
                  <dd><span class="ng-scope"></span></dd>
                  <dt class="ng-binding">+86 138****9453</dt>
                </dl>
                <ul>
                  <!--<li class="order"><a href="javascript:void(0)">我的订单</a></li>-->
                  <router-link tag="li" class="order" :to="{name:'Account'}"><a>我的订单</a></router-link>
                  <li class="support"><a href="javascript:void(0)">售后服务</a></li>
                  <li class="coupon"><a href="javascript:void(0)">我的优惠</a></li>
                  <li class="information"><a href="javascript:void(0)">账户资料</a></li>
                  <li class="address"><a href="javascript:void(0)">收货地址</a></li>
                  <li class="logout"><a href="javascript:void(0)">退出</a></li>
                </ul>
              </div>
            </div>
          </li>
          <!--active-->
          <car-panel></car-panel>
        </ul>
        <ul class="nav-list">
          <li><a href="javascript:void(0)">在线商城</a></li>
          <li><a href="javascript:void(0)">坚果 Pro</a></li>
          <li><a href="javascript:void(0)">Smartisan M1 / M1L</a></li>
          <li><a href="javascript:void(0)">Smartisan OS</a></li>
          <li><a href="javascript:void(0)">欢喜云</a></li>
          <li><a href="javascript:void(0)">应用下载</a></li>
          <li><a href="javascript:void(0)">官方论坛</a></li>
        </ul>
      </div>
    </div>
    <div class="nav-sub">
      <div class="nav-sub-wrapper">
        <div class="container">
          <ul class="nav-list">
            <li><a href="javascript:void(0)">首页</a></li>
            <li><a href="javascript:void(0)">手机</a></li>
            <li><a href="javascript:void(0)">“足迹系列”手感膜</a></li>
            <li class="active"><a href="javascript:void(0)">官方配件</a></li>
            <li><a href="javascript:void(0)">周边产品</a></li>
            <li><a href="javascript:void(0)">第三方配件</a></li>
            <li><a href="javascript:void(0)">全部商品</a></li>
            <li><a href="javascript:void(0)">服务</a></li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import carPanel from '@/components/car-panel'
export default {
  name: 'header-nav',
  components: {
    carPanel
  }
}
</script>

<style scoped>

</style>
