    ...
    <div class="module-dialog clear module-dialog-address module-dialog-show">
      <div class="dialog-panel">
        <div class="topbar">
          ...
          <span class="dialog-close" @click="hideAddressPop">x</span>
        </div>
              ...
              <div class="save-address-box">
                <div class="address-form">
                  <div class="module-form-row">
                    <div class="form-item-v3" :class="{'form-invalid-item': errors.name}">
                      <input type="text" placeholder="收货人姓名" class="js-verify" v-model="newReceive.name" @blur="inspectName">
                      <div class="verify-error" v-show="errors.name">姓名格式错误</div>
                    </div>
                  </div>
                  <div class="module-form-row">
                    <div class="form-item-v3" :class="{'form-invalid-item': errors.phone}">
                      <input type="text" placeholder="手机号" class="js-verify" v-model="newReceive.phone" @blur="inspectPhone">
                      <div class="verify-error" v-show="errors.phone">手机号格式错误</div>
                    </div>
                  </div>
                  <div class="module-form-row clear">
                    <div class="form-item-v3 area-code-w fn-left form-valid-item">
                      <input type="text" placeholder="区号（可选）" class="js-verify js-address-area-code" v-model="newReceive.areaCode">
                      <div class="verify-error"></div>
                    </div>
                    <div class="form-item-v3 telephone-w fn-right form-valid-item">
                      <input type="text" placeholder="固定电话（可选）" class="js-verify js-address-telephone" v-model="newReceive.landLine">
                      <div class="verify-error"></div>
                    </div>
                  </div>
                  <div class="module-form-row clear">
                    <div class="form-item-v3 select-item province-wrapper">
                      <select name="province_code" class="province select-province js-form-province js-verify" v-model="newReceive.provinceId">
                        <option value="0">请选择省份</option>
                        <option :value="province.area_id" v-for="(province, index) in addList" v-bind:key="index">{{province.area_name}}</option>
                  ...
                  <div class="module-form-row clear">
                    <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                      <select class="city select-city js-form-city js-verify" v-model="newReceive.cityId">
                        <option value="0">请选择城市</option>
                        <option :value="city.area_id" v-for="(city, index) in cityList" v-bind:key="index">{{city.area_name}}</option>
                      </select>
                    </div>
                    <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                      <select class="city select-city js-form-city js-verify" v-model="newReceive.countyId">
                        <option value="0">请选择区县</option>
                        <option :value="county.area_id" v-for="(county, index) in countyList" v-bind:key="index">{{county.area_name}}</option>
                      </select>
                    </div>
                  </div>
                  <div class="module-form-row">
                    <div class="form-item-v3">
                      <input type="text" placeholder="详细地址，如街道名称，楼层，门牌号码等" class="js-verify" v-model="newReceive.add">
                      <div class="verify-error"></div>
                    </div>
                  </div>
                  <div class="module-form-row fn-clear">
                    <input type="checkbox" class="hide" value="1">
                    <span class="blue-checkbox" :class="{'blue-checkbox-on': newReceive.default}" @click="checkDefault"></span>设为默认
                  </div>
...
<script>
import addList from '@/lib/addLists'
export default {
  name: 'address-pop',
  data () {
    return {
  ...
  watch: {
    'newReceive.provinceId' (curVal, oldVal) {
      this.setCityId(0)
      this.setCountyId(0)
    },
    'newReceive.cityId' (curVal, oldVal) {
      this.setCountyId(0)
    },
    'newReceive.countyId' (curVal, oldVal) {
      this.countyList.forEach((county) => {
        if (county.area_id === curVal) {
          this.setCounty(county.area_name)
        }
      })
    },
    // 深度监控
  ...
  computed: {
    cityList () {
      ...
      this.addList.forEach((province) => {
        if (province.area_id === this.newReceive.provinceId) {
          cities = province.city_list
          this.setProvince(province.area)
        }
      })
      return cities
    },
    countyList () {
      ...
      this.cityList.forEach((city) => {
        if (city.area_id === this.newReceive.cityId) {
          countries = city.county_list
          this.setCity(city.area_name)
        }
      })
      return countries
    }
  ...
  methods: {
    ...
    inspectPhone () {
      this.errors.phone = !(/^1[34578]\d{9}$/.test(this.newReceive.phone))
    },
    inspectName () {
      this.errors.name = !(this.newReceive.name.length > 0)
    },
    checkDefault () {
      this.newReceive.default = !this.newReceive.default
    },
    setProvince (province) {
      this.newReceive.province = province
    },
    setCity (city) {
      this.newReceive.city = city
    },
    setCounty (county) {
      this.newReceive.county = county
    },
    setProvinceId (province) {
      this.newReceive.provinceId = province
    },
    setCityId (city) {
      this.newReceive.cityId = city
    },
    setCountyId (county) {
      this.newReceive.countyId = county
    },
    inspectReceive () {
      this.isRight = (!this.errors.name && !this.errors.phone && this.newReceive.provinceId && this.newReceive.cityId && this.newReceive.countyId && this.newReceive.add)
    },
    submitReceiveHandle () {
      if (this.isRight) {
        this.$store.commit('submitReceive', this.newReceive)
        this.$emit('close')
      }
    }
  }
