    ...
    <div>
      ...
      <div class="params-colors">
        <ul class="colors-list">
          <li v-for="(sku,index) in item.sku_info" v-bind:key="index">
            <a href="javascript:void(0)" :class="{'active':index==itemIndex}" @mouseenter="tableIndex(index)"><img :src="'http://img01.smartisanos.cn/'+sku.spec_json.image+'20X20.jpg'" :title="sku.spec_json.show_name"></a>
          </li>
        </ul>
      </div>
      <div class="item-btns clearfix">
        <span class="item-gray-btn"><router-link :to="{name:'Item', query: {itemId:item.sku_info[itemIndex].sku_id}}">查看详情</router-link></span><span class="item-blue-btn" @click="addCarPanelHandle(item.sku_info[itemIndex])">加入购物车 </span>
      </div>
...
export default {
  ...
  methods: {
    ...
    addCarPanelHandle (data) {
      let goodsData = {'info': data, 'count': 1}
      this.$store.commit('addCarPanelData', goodsData)
    }
  }
}
</script>
